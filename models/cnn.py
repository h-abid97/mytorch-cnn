...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(self.nlayers):
            conv_layer = Conv1d(num_input_channels, num_channels[i], kernel_sizes[i], strides[i])
    ...
    def backward(self, labels):
        ...
        reversed_conv_layers = reversed(self.convolutional_layers)
        reversed_activations = reversed(self.activations)

        # Iterate over the layers and activations in pairs
        for conv_layer, activation in zip(reversed_conv_layers, reversed_activations):
            # Apply the backward method of the activation layer to the gradient
            grad = grad * activation.backward()
            # Apply the backward method of the convolutional layer to the updated gradient
